/**
 * Did You Mean API - Error Handling Example
 *
 * This example demonstrates proper error handling when using the Did You Mean API.
 * API Documentation: https://docs.apiverve.com/ref/didyoumean
 */

const API_KEY = process.env.APIVERVE_API_KEY || 'YOUR_API_KEY_HERE';
const API_URL = 'https://api.apiverve.com/v1/didyoumean';
 ...
 */
async function callDidYouMeanAPIWithErrorHandling(queryParams = {}) {
  try {
...
async function callWithRetry(maxRetries = 3, initialDelay = 1000) {
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      const result = await callDidYouMeanAPIWithErrorHandling({
